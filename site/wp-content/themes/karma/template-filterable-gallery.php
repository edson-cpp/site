...
$count++; $col ++; $mod = ($count % $no_of_columns == 0) ? 0 : $no_of_columns - $count % $no_of_columns;
...
$image_src = truethemes_crop_image($thumb,$external_image_url,$image_width,$image_height);
		...
		$portfolio_img .= "<img class='tt-fadein' src='$image_src' width='$image_width' height='$image_height' alt='$title' />";
...
<div data-id="id-<?php echo absint( $count ); ?>" class="port-1-wrap <?php if ( $terms ) : foreach ( $terms as $term ) : echo sanitize_html_class( 'term-' . absint( $term->term_id ) ) . ' '; endforeach; endif; ?>">
		...
		<div class="portfolio_one_column<?php if ($gallery_layout == 'tt-1-col'): echo ' gallery_single';endif;?>">
			<?php 	if('yes' == $content_title):echo '<h3>'.$gallery_title.'</h3>';endif;
					if('yes' == $content_description):echo '<p>'.$gallery_description.'</p>';endif; //end title/description check ?>
...
<?php else: //else 2 column, 3 column, 3 column portrait, and 4 column... ?>
		...
		<div data-id="id-<?php echo absint( $count ); ?>" class="<?php echo $tt_column_size; if($col == $no_of_columns){  echo '_last'; $col = 0; } ?> <?php if ( $terms ) : foreach ( $terms as $term ) : echo sanitize_html_class( 'term-' . absint( $term->term_id ) ) . ' '; endforeach; endif; ?>">
